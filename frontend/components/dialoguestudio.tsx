  ...
  const renderViaWebSocket = (
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.onmessage = (evt) => {
        ...
        if (msg.status === "ping") return; // keepalive, ignore
...
function ResultBox({ result, apiUrl, ngrokHeader }: { result: RenderResult; apiUrl: string; ngrokHeader: Record<string, string> }) {
  ...
  // For direct download — use a fetch with headers to bypass tunnel warning,
  // but stream it as a download instead of loading into memory
  const handleDownload = async () => {
    try {
      const res = await fetch(`${apiUrl}${result.download_url}`, { headers: ngrokHeader });
      ...
      // Fallback: open directly in new tab if fetch fails
      window.open(`${apiUrl}${result.download_url}`, "_blank");
    }
  };

  // Build audio src with cache-busting to avoid tunnel warning on audio tag
  const audioSrc = `${apiUrl}${result.download_url}`;
  ...
  return (
    <div style={s.resultBox}>
      <div style={s.row}>
        ...
        <button onClick={handleDownload} style={{ ...s.downloadBtn, border: "none", cursor: "pointer" }}>
          ⬇ Download WAV
        </button>
      </div>
